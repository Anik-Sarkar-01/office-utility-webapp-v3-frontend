...
const EditEmployee = (data) => {
  ...
  const onFinish = async (values) => {
    const { username, email, position, upload, phone, address } =
      values;
    ...
    try {
      await axios.patch(`https://office-utility-webapp-v3-backend.vercel.app/api/users/editEmployee/${user._id}`, formData, {
        headers: {
        ...
        >
          ...
          <Form.Item
            ...
            rules={[
              {
                required: true,
                pattern: new RegExp(/^(\+?8801|01)[3-9]\d{8}$/),
